import sqlite3
import struct
import urllib.parse
from http.server import BaseHTTPRequestHandler, HTTPServer

DB_FILE = "yufs.db"
SERVER_PORT = 8080
ROOT_INO = 1000

# Режимы доступа
S_IFDIR = 0o040000
S_IFREG = 0o100000

def get_db():
    conn = sqlite3.connect(DB_FILE)
    conn.row_factory = sqlite3.Row
    return conn

def init_fs():
    with get_db() as conn:
        conn.executescript("""
                           CREATE TABLE IF NOT EXISTS inodes (
                                                                 id INTEGER PRIMARY KEY, mode INTEGER, nlink INTEGER DEFAULT 1, size INTEGER DEFAULT 0, content BLOB
                           );
                           CREATE TABLE IF NOT EXISTS dirents (
                                                                  parent_id INTEGER, name TEXT, inode_id INTEGER,
                                                                  PRIMARY KEY(parent_id, name)
                               );
                           """)
        if not conn.execute("SELECT 1 FROM inodes WHERE id=?", (ROOT_INO,)).fetchone():
            conn.execute("INSERT INTO inodes (id, mode, nlink, size) VALUES (?, ?, 1, 0)", (ROOT_INO, S_IFDIR | 0o777))

class YUFSHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ret_val = -1
        body = b""
        try:
            # Парсим URL
            parsed = urllib.parse.urlparse(self.path)
            cmd = parsed.path.replace("/api/", "")
            qs = urllib.parse.parse_qs(parsed.query)
            args = {k: v[0] for k, v in qs.items()}

            with get_db() as conn:
                method = getattr(self, f"handle_{cmd}", None)
                if method:
                    ret_val, body = method(conn, args)
                else:
                    print(f"Unknown command: {cmd}")
        except Exception as e:
            print(f"Error: {e}")
            ret_val = -1
            body = b"" # При ошибке тело пустое

        # Отправляем 8 байт (статус) + данные
        response = struct.pack('<q', ret_val) + body
        self.send_response(200)
        self.send_header('Content-Length', str(len(response)))
        self.end_headers()
        self.wfile.write(response)

    # --- Вспомогательные функции ---
    def pack_stat(self, id, mode, size):
        # struct YUFS_stat { uint32_t id; umode_t mode; uint64_t size; }
        return struct.pack('<IIQ', id, mode, size)

    # --- Обработчики ---

    def handle_lookup(self, conn, args):
        row = conn.execute("SELECT i.id, i.mode, i.size FROM dirents d JOIN inodes i ON d.inode_id = i.id WHERE d.parent_id=? AND d.name=?",
                           (args['parent_id'], args['name'])).fetchone()
        if row:
            return 0, self.pack_stat(row['id'], row['mode'], row['size'])
        return -1, b""

    def handle_create(self, conn, args):
        mode = int(args['mode'])
        try:
            cur = conn.execute("INSERT INTO inodes (mode, size, content) VALUES (?, 0, NULL)", (mode,))
            new_id = cur.lastrowid
            conn.execute("INSERT INTO dirents (parent_id, name, inode_id) VALUES (?, ?, ?)",
                         (int(args['parent_id']), args['name'], new_id))
            return 0, self.pack_stat(new_id, mode, 0)
        except:
            return -1, b""

    def handle_link(self, conn, args):
        try:
            # Проверяем, что цель и родитель существуют и типы корректны
            target = conn.execute("SELECT mode FROM inodes WHERE id=?", (int(args['target_id']),)).fetchone()
            if not target or (target['mode'] & S_IFDIR): return -1, b""

            conn.execute("INSERT INTO dirents (parent_id, name, inode_id) VALUES (?, ?, ?)",
                         (int(args['parent_id']), args['name'], int(args['target_id'])))
            conn.execute("UPDATE inodes SET nlink = nlink + 1 WHERE id=?", (int(args['target_id']),))
            return 0, b""
        except:
            return -1, b""

    def handle_unlink(self, conn, args):
        try:
            d = conn.execute("SELECT inode_id FROM dirents WHERE parent_id=? AND name=?", (int(args['parent_id']), args['name'])).fetchone()
            if not d: return -1, b""

            conn.execute("DELETE FROM dirents WHERE parent_id=? AND name=?", (int(args['parent_id']), args['name']))
            # Логику уменьшения nlink опустим для простоты, главное удалить запись
            return 0, b""
        except:
            return -1, b""

    def handle_rmdir(self, conn, args):
        try:
            conn.execute("DELETE FROM dirents WHERE parent_id=? AND name=?", (int(args['parent_id']), args['name']))
            return 0, b""
        except:
            return -1, b""

    def handle_getattr(self, conn, args):
        row = conn.execute("SELECT id, mode, size FROM inodes WHERE id=?", (int(args['id']),)).fetchone()
        if row:
            return 0, self.pack_stat(row['id'], row['mode'], row['size'])
        return -1, b""

    def handle_read(self, conn, args):
        offset = int(args['offset'])
        size = int(args['size'])
        row = conn.execute("SELECT content FROM inodes WHERE id=?", (int(args['id']),)).fetchone()

        content = row['content'] if row and row['content'] else b''
        if offset >= len(content): return 0, b"" # EOF

        chunk = content[offset : offset + size]
        return len(chunk), chunk

    def handle_write(self, conn, args):
        try:
            inode_id = int(args['id'])
            offset = int(args['offset'])
            buf = urllib.parse.unquote_to_bytes(args['buf'])

            row = conn.execute("SELECT content FROM inodes WHERE id=?", (inode_id,)).fetchone()
            content = bytearray(row['content']) if row and row['content'] else bytearray()

            new_end = offset + len(buf)
            if new_end > len(content): content.extend(b'\0' * (new_end - len(content)))
            content[offset : offset + len(buf)] = buf

            conn.execute("UPDATE inodes SET content=?, size=? WHERE id=?", (content, len(content), inode_id))
            return len(buf), b""
        except:
            return -1, b""

    def handle_iterate(self, conn, args):
        inode_id = int(args['id'])
        offset = int(args['offset'])

        entries = []
        entries.append((inode_id, ".", S_IFDIR | 0o777))
        entries.append((inode_id, "..", S_IFDIR | 0o777))

        rows = conn.execute("SELECT d.name, d.inode_id, i.mode FROM dirents d JOIN inodes i ON d.inode_id=i.id WHERE d.parent_id=?", (inode_id,)).fetchall()
        for r in rows: entries.append((r['inode_id'], r['name'], r['mode']))

        if offset >= len(entries):
            return -1, b"" # ВАЖНО: -1 прерывает цикл в C

        e = entries[offset]
        name_bytes = e[1].encode('utf-8')
        packed = struct.pack('<I256sI', e[0], name_bytes, e[2])
        return 0, packed

if __name__ == '__main__':
    init_fs()
    server = HTTPServer(('0.0.0.0', SERVER_PORT), YUFSHandler)
    print(f"Simple YUFS Backend running on port {SERVER_PORT}...")
    server.serve_forever()